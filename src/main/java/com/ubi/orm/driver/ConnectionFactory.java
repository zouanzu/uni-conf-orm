...
public class ConnectionFactory {
    ...
    private ConnectionFactory() {
        // 注册各数据库驱动的单例实例
        connections.put("mysql", MySQLConnection.getInstance());
        connections.put("mssql", MSSQLConnection.getInstance());
        connections.put("sqlite", SQLiteConnection.getInstance());
    }
    ...
    public static ConnectionFactory getInstance() {
        return ConnectionFactory.SingletonHolder.INSTANCE;
    }
     ...
     */
    public DatabaseConnection getConnection(String driveType) {
        DatabaseConnection connection = connections.get(driveType);
        if (connection == null) {
     ...
     * 动态注册新的数据库驱动（扩展用）
     * @param type 数据库类型标识
     * @param connection 驱动实例
     */
    public void registerDriver(String type, DatabaseConnection connection) {
